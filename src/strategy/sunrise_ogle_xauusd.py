...
DATA_FILENAME = 'XAUUSD_5m_5Yea.csv'     # 🥇 Gold (XAU/USD) 5-minute data
...
ENABLE_SHORT_TRADES = True            # Enable short (sell) entries - ENABLED FOR PROP FIRM

# === DUAL CEREBRO MODE ===
RUN_DUAL_CEREBRO = False             # Run separate LONG-only and SHORT-only cerebros to avoid position interference

# === DEBUG SETTINGS ===
VERBOSE_DEBUG = True                 # Print detailed debug info to console (set True only for troubleshooting)
    ...
    def _init_trade_reporting(self):
        ...
        if EXPORT_TRADE_REPORTS or TRADE_REPORT_ENABLED:
            try:
                ...
                print(f"WARNING: Trade reporting initialization failed: {e}")
                self.trade_report_file = None
...
if __name__ == '__main__':
    ...
    if RUN_DUAL_CEREBRO and ENABLE_LONG_TRADES and ENABLE_SHORT_TRADES:
        ...
        # Add analyzers for LONG
        cerebro_long.addanalyzer(bt.analyzers.SharpeRatio, _name='sharpe', timeframe=bt.TimeFrame.Days, riskfreerate=0.0)
        cerebro_long.addanalyzer(bt.analyzers.DrawDown, _name='drawdown')
        cerebro_long.addanalyzer(bt.analyzers.TradeAnalyzer, _name='trades')
        cerebro_long.addanalyzer(bt.analyzers.Returns, _name='returns')
        ...
        # Add analyzers for SHORT
        cerebro_short.addanalyzer(bt.analyzers.SharpeRatio, _name='sharpe', timeframe=bt.TimeFrame.Days, riskfreerate=0.0)
        cerebro_short.addanalyzer(bt.analyzers.DrawDown, _name='drawdown')
        cerebro_short.addanalyzer(bt.analyzers.TradeAnalyzer, _name='trades')
        cerebro_short.addanalyzer(bt.analyzers.Returns, _name='returns')
        ...
        print(f"\n{'='*70}")
        print(f"DUAL CEREBRO PERFORMANCE METRICS - XAUUSD STRATEGY")
        print(f"{'='*70}")
        
        # LONG Strategy Metrics
        print(f"\n LONG-ONLY STRATEGY:")
        print(f"  Portfolio Value: ${final_value_long:,.2f}")
        print(f"  P&L: ${long_pnl:+,.2f}")
        print(f"  Return: {(long_pnl / STARTING_CASH) * 100:.2f}%")
        
        long_sharpe = long_strategy.analyzers.sharpe.get_analysis().get('sharperatio', None)
        long_dd = long_strategy.analyzers.drawdown.get_analysis()
        long_max_dd = long_dd.get('max', {}).get('drawdown', 0)
        long_trades_analysis = long_strategy.analyzers.trades.get_analysis()
        long_total = long_trades_analysis.get('total', {}).get('total', 0)
        long_won = long_trades_analysis.get('won', {}).get('total', 0)
        long_gross_profit = long_trades_analysis.get('won', {}).get('pnl', {}).get('total', 0)
        long_gross_loss = abs(long_trades_analysis.get('lost', {}).get('pnl', {}).get('total', 0))
        long_pf = long_gross_profit / long_gross_loss if long_gross_loss > 0 else float('inf')
        
        print(f"  Trades: {long_total} | Wins: {long_won} | Win Rate: {(long_won/long_total*100) if long_total > 0 else 0:.2f}%")
        print(f"  Profit Factor: {long_pf:.2f}")
        print(f"  Max Drawdown: {long_max_dd:.2f}%")
        print(f"  Sharpe Ratio: {long_sharpe if long_sharpe is not None else 'N/A'}")
        
        # SHORT Strategy Metrics
        print(f"\n SHORT-ONLY STRATEGY:")
        print(f"  Portfolio Value: ${final_value_short:,.2f}")
        print(f"  P&L: ${short_pnl:+,.2f}")
        print(f"  Return: {(short_pnl / STARTING_CASH) * 100:.2f}%")
        
        short_sharpe = short_strategy.analyzers.sharpe.get_analysis().get('sharperatio', None)
        short_dd = short_strategy.analyzers.drawdown.get_analysis()
        short_max_dd = short_dd.get('max', {}).get('drawdown', 0)
        short_trades_analysis = short_strategy.analyzers.trades.get_analysis()
        short_total = short_trades_analysis.get('total', {}).get('total', 0)
        short_won = short_trades_analysis.get('won', {}).get('total', 0)
        short_gross_profit = short_trades_analysis.get('won', {}).get('pnl', {}).get('total', 0)
        short_gross_loss = abs(short_trades_analysis.get('lost', {}).get('pnl', {}).get('total', 0))
        short_pf = short_gross_profit / short_gross_loss if short_gross_loss > 0 else float('inf')
        
        print(f"  Trades: {short_total} | Wins: {short_won} | Win Rate: {(short_won/short_total*100) if short_total > 0 else 0:.2f}%")
        print(f"  Profit Factor: {short_pf:.2f}")
        print(f"  Max Drawdown: {short_max_dd:.2f}%")
        print(f"  Sharpe Ratio: {short_sharpe if short_sharpe is not None else 'N/A'}")
        
        # COMBINED Metrics
        print(f"\n COMBINED STRATEGY (LONG + SHORT):")
        print(f"  Combined Portfolio Value: ${combined_value:,.2f}")
        print(f"  Combined P&L: ${combined_pnl:+,.2f}")
        print(f"  Combined Return: {(combined_pnl / STARTING_CASH) * 100:.2f}%")
        print(f"  Total Trades: {combined_trades}")
        print(f"  Winning Trades: {combined_wins}")
        print(f"  Losing Trades: {combined_losses}")
        print(f"  Win Rate: {combined_win_rate:.2f}%")
        print(f"  Combined Profit Factor: {combined_pf:.2f}")
        print(f"  Gross Profit: ${combined_gross_profit:,.2f}")
        print(f"  Gross Loss: ${abs(combined_gross_loss):,.2f}")
        
        # Calculate combined max drawdown (worst of the two)
        combined_max_dd = max(long_max_dd, short_max_dd)
        print(f"  Max Drawdown (worst): {combined_max_dd:.2f}%")
        ...
        # Add performance analyzers (like reference file)
    ...
    print(f"\n{'='*70}")
    print(f"PERFORMANCE METRICS - XAUUSD STRATEGY")
    print(f"{'='*70}")
    print(f"Final Portfolio Value: ${final_value:,.2f}")
    print(f"Starting Cash: ${STARTING_CASH:,.2f}")
    print(f"Total P&L: ${final_value - STARTING_CASH:+,.2f}")
    print(f"Return: {((final_value / STARTING_CASH) - 1) * 100:.2f}%")
    
    # Get analyzer results - debug analyzer access
    strategy_result = results[0]
    
    # Access analyzers using getbyname method
    try:
        sharpe_analyzer = strategy_result.analyzers.getbyname('sharpe').get_analysis()
        drawdown_analyzer = strategy_result.analyzers.getbyname('drawdown').get_analysis()
        trade_analyzer = strategy_result.analyzers.getbyname('trades').get_analysis()
    except Exception as e:
        print(f"DEBUG: Error with getbyname(): {e}")
        # Fallback to indexed access
        if len(strategy_result.analyzers) >= 3:
            sharpe_analyzer = strategy_result.analyzers[0].get_analysis()
            drawdown_analyzer = strategy_result.analyzers[1].get_analysis()
            trade_analyzer = strategy_result.analyzers[2].get_analysis()
        else:
            print("ERROR: Not enough analyzers added!")
            sharpe_analyzer = {}
            drawdown_analyzer = {}
            trade_analyzer = {}
    ...
    print(f"\nSharpe Ratio: {sharpe_ratio:.3f}" if sharpe_ratio else "\nSharpe Ratio: N/A")
    
    # Calculate Max Drawdown
    max_dd_raw = drawdown_analyzer.get('max', {}).get('drawdown', 0)
    max_dd_money = drawdown_analyzer.get('max', {}).get('moneydown', 0)
    ...
    # Trade Analysis and Profit Factor (like reference file)
    ...
    if total_trades > 0:
        ...
        # Profit Factor calculation (like reference file)
